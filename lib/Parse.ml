...
let children_regexps : (string * Run.exp option) list = [
  ...
  Some (
    Alt [|
      ...
      Token (Name "metavariable_keyword");
    |];
  );
  "keywords",
  Some (
...
let trans_newline_before_comment ((kind, body) : mt) : CST.newline_before_comment =
  match body with
  | Leaf v -> v
  | Children _ -> assert false
...
let trans_newline_before_binary_operator ((kind, body) : mt) : CST.newline_before_binary_operator =
  match body with
  | Leaf v -> v
  | Children _ -> assert false
...
and trans_atom ((kind, body) : mt) : CST.atom =
  ...
  | Children v ->
      ...
      | Alt (2, v) ->
          `Meta_atom (
            trans_metavariable_atom (Run.matcher_token v)
          )
      | _ -> assert false
      )
  | Leaf _ -> assert false
...
and trans_keyword ((kind, body) : mt) : CST.keyword =
  ...
  | Children v ->
      ...
      | Alt (2, v) ->
          `Meta_kw (
            trans_metavariable_keyword (Run.matcher_token v)
          )
      | _ -> assert false
      )
  | Leaf _ -> assert false
