...
let map_imm_tok_pat_b250293 (env : env) (tok : CST.imm_tok_pat_b250293) =
  (* pattern [A-Z][A-Z0-9]* *) token env tok
...
let map_operator_identifier (env : env) (x : CST.operator_identifier) =
  ...
  | `HATHATHAT tok -> R.Case ("HATHATHAT",
      (* "^^^" *) token env tok
    )
...
and map_expression (env : env) (x : CST.expression) =
  ...
  | `Sigil (v1, v2, v3) -> R.Case ("Sigil",
      ...
      let v2 =
        ...
        | `Imm_tok_pat_b250293_choice_quoted_double (v1, v2) -> R.Case ("Imm_tok_pat_b250293_choice_quoted_double",
            let v1 = map_imm_tok_pat_b250293 env v1 in
            let v2 =
              (match v2 with
  ...
  | `Anon_func (v1, v2, v3, v4) -> R.Case ("Anon_func",
      ...
      let v3 =
        (match v3 with
        | Some (v1, v2) -> R.Option (Some (
            let v1 = map_stab_clause env v1 in
            let v2 =
              R.List (List.map (fun (v1, v2) ->
                let v1 = map_terminator env v1 in
