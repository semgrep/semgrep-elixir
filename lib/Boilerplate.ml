...
and map_anon_choice_choice_stab_clause_rep_term_choice_stab_clause_b295119 (env : env) (x : CST.anon_choice_choice_stab_clause_rep_term_choice_stab_clause_b295119) =
  ...
  | `Choice_stab_clause_rep_term_choice_stab_clause (v1, v2) ->
      ...
      in
      let v2 =
        List.map (map_anon_term_choice_stab_clause_70647b7 env) v2
      in
      todo env (v1, v2)
  | `Choice_exp_rep_term_choice_exp_opt_term (v1, v2, v3) ->
      ...
      in
      let v2 =
        List.map (map_anon_term_choice_exp_996111b env) v2
      in
      let v3 =
        (match v3 with
        | Some x -> map_terminator env x
...
and map_call_with_parentheses (env : env) (x : CST.call_with_parentheses) =
  (match x with
  | `Local_call_with_parens x ->
      map_local_call_with_parentheses env x
  | `Remote_call_with_parens x ->
...
and map_expression (env : env) (x : CST.expression) =
  ...
  | `Sigil (v1, v2, v3) ->
      ...
      let v3 =
        (match v3 with
        | Some x -> map_imm_tok_pat_8f9e87e env x
        | None -> todo env ())
      in
      todo env (v1, v2, v3)
  | `List (v1, v2, v3) ->
